...
import com.ch.aidu.helper.BookRecordHelper;
import com.ch.aidu.utils.DataUtils;
import com.ch.aidu.utils.ScreenUtils;
import com.ch.aidu.utils.ToastUtils;
...
//        }
    ...
    public void setTextSize(int textSize) {
        ...
        if (mStatus == STATUS_FINISH) {
            ...
            if (mCurPage.getPosition() >= mCurPageList.size()) {
                mCurPage.setPosition(mCurPageList.size() - 1);
            }
        }
        //重新设置文章指针的位置
        mCurPage = getCurPage(mCurPage.getPosition());
        //绘制
        mPageView.refreshPage();
    }
    ...
    public void setBgColor(int theme) {
        if (isNightMode && theme == ReadSettingManager.NIGHT_MODE) {
            mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c99ffffff);
            mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c000000);
        } else if (isNightMode) {
            mBgTheme = theme;
            ...
            switch (theme) {
                case ReadSettingManager.READ_BG_DEFAULT:
                    mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c2c2c2c);
                    mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.cddcec29c);
                    break;
                case ReadSettingManager.READ_BG_1:
                    mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c2f332d);
                    mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.cccebcc);
                    break;
                case ReadSettingManager.READ_BG_2:
                    mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c92918c);
                    mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.caaaaaa);
                    break;
                case ReadSettingManager.READ_BG_3:
                    mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c383429);
                    mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.cd1cec5);
                    break;
                case ReadSettingManager.READ_BG_4:
                    mTextColor = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c627176);
                    mPageBg = ContextCompat.getColor(MyApplication.getAppContext(), R.color.c001c27);
                    break;
            }
        }
    ...
    public void saveRecord() {
        ...
        mBookRecord.setPagePos(mCurPage.getPosition());
    ...
    void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (mStatus != STATUS_FINISH) {
                ...
                canvas.drawText(mCurPage.getTitle(), mMarginWidth, tipTop, mTipPaint);
            }
            ...
            //只有finish的时候采用页码
            if (mStatus == STATUS_FINISH) {
                String percent = (mCurPage.getPosition() + 1) + "/" + mCurPageList.size();
        ...
        //底部的字显示的位置Y
        float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom - tipMarginHeight;
        String time = DataUtils.dateConvert(System.currentTimeMillis(), "HH:mm");
    ...
    void drawContent(Bitmap bitmap) {
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            //对标题进行绘制
            for (int i = 0; i < mCurPage.getTitleLines(); ++i) {
            ...
            //对内容进行绘制
            for (int i = mCurPage.getTitleLines(); i < mCurPage.getLines().size(); ++i) {
                str = mCurPage.getLines().get(i);
    ...
    void pageCancel() {
        //加载到下一章取消了
        if (mCurPage.getPosition() == 0 && mCurChapterPos > mLastChapter) {
            prevChapter();
        }
        //加载上一章取消了 (可能有点小问题)
        else if (mCurPageList == null ||
                (mCurPage.getPosition() == mCurPageList.size() - 1 && mCurChapterPos < mLastChapter)) {
            nextChapter();
        }
